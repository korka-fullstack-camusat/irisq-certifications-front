...
function hasExamResult(r: Row): boolean {
    return !!(r.exam_status || r.final_grade || r.final_appreciation || r.exam_grade);
}

function getMotif(r: Row): { label: string; isRetry: boolean } {
    const es = (r.exam_status || "").toString().trim();
    const esLow = es.toLowerCase();
    if (esLow === "rattrapage") return { label: "Rattrapage", isRetry: true };
    if (esLow.includes("non acquis")) return { label: "Non acquis", isRetry: false };
    if (es) return { label: es, isRetry: false };
    if (r.final_appreciation) return { label: r.final_appreciation, isRetry: false };
    return { label: "Examen non passé", isRetry: false };
}

function CandidatsNonCertifiesContent() {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                setRows(merged.filter(r => r.status === "approved" && hasExamResult(r) && !isExamPassed(r)));
            } catch (e) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                    <h1 className="text-2xl font-black flex items-center gap-2" style={{ color: "#1a237e" }}>
                        <XCircle className="h-6 w-6" style={{ color: "#c62828" }} />
                        Candidats non certifiés
                    </h1>
                    <p className="text-sm text-gray-500 mt-1">
                        Candidats validés n&apos;ayant pas réussi leur examen, avec le motif associé.
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {([
                    { key: "all",    label: "Tous les non certifiés", count: baseFiltered.length, icon: Users,   color: "#c62828", bg: "#fef2f2", desc: "Vue globale"      },
                    { key: "online", label: "Non certifiés en ligne",  count: onlineCount,         icon: Monitor, color: "#1a237e", bg: "#e8eaf6", desc: "Examen à distance" },
                    { key: "onsite", label: "Non certifiés présentiel",count: onsiteCount,         icon: MapPin,  color: "#2e7d32", bg: "#e8f5e9", desc: "Examen sur site"   },
                ] as const).map(t => {
                    const Icon = t.icon;
            ...
            <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm flex flex-wrap items-center gap-3">
                ...
                <label className="inline-flex items-center gap-2 text-xs text-gray-600">
                    <XCircle className="h-3 w-3" style={{ color: "#c62828" }} />
                    <select
                        value={formation}
            ...
            {loading ? (
                ...
                <div className="p-12 text-center rounded-2xl bg-white border border-dashed border-gray-200">
                    ...
                    <p className="text-gray-500 text-sm">Aucun candidat non certifié ne correspond à ces critères.</p>
                </div>
            ) : (
                <>
                            ...
                            return (
                                ...
                                >
                                    <Link
                                        ...
                                        className="group flex items-center gap-4 p-4 rounded-2xl bg-white border border-gray-100 shadow-sm hover:shadow-md hover:border-red-200 transition-all"
                                    >
                                        <div
                                        ...
                                        <ChevronRight className="h-5 w-5 text-gray-300 group-hover:text-red-400 transition-colors shrink-0" />
                                    </Link>
                                </motion.div>
                            );
                        })}
...
export default function CandidatsNonCertifiesPage() {
    return (
        <Suspense fallback={null}>
            <CandidatsNonCertifiesContent />
        </Suspense>
    );
}
