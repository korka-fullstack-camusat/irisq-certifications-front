...
import { fetchResponse, type DocumentValidationEntry } from "@/lib/api";
...
export default function ValidatedCandidateDossierPage() {
    ...
    return (
        <div className="space-y-6">
            <Link
                href="/dashboard/candidatures-validees"
                className="inline-flex items-center gap-1 text-xs font-bold text-gray-500 hover:text-gray-700"
            >
                <ArrowLeft className="h-3 w-3" /> Retour aux candidatures validées
            </Link>
                ...
                return (
                    <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex items-center gap-4 flex-wrap">
                        ...
                        <button
                            type="button"
                            onClick={() => {
                                if (!canSendCertificate) return;
                                alert("Envoi du certificat — à implémenter.");
                            }}
                            disabled={!canSendCertificate}
                            title={canSendCertificate ? "Envoyer le certificat au candidat" : "Disponible uniquement lorsque l'examen est réussi"}
                        ...
                        {availableDocs.map(doc => {
                            ...
                            return (
                                <div
                                    ...
                                    style={{
                                        borderColor: isValid ? "#c8e6c9" : "#e5e7eb",
                                        backgroundColor: isValid ? "#f1f8f4" : "#f9fafb",
                                    }}
                                >
